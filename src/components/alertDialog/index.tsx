    ...
    <AlertDialog.Root onOpenChange={(state) => setIsOpen(state)}>
      ...
      <AlertDialog.Portal>
        <AlertDialog.Overlay
          ...
          } transition-all duration-150 ease-in-out flex fixed inset-0 justify-center items-center p-4 w-full h-full bg-opacity-10 drop-shadow-lg backdrop-blur-xl bg-slate-900 z-[99]`}>
          <AlertDialog.Content
            className={`${
              isOpen ? "opacity-100 scale-100" : "opacity-0 scale-0"
            } p-4 w-full md:w-[500px]  md:text-base text-sm rounded-md text-slate-900 bg-slate-50 transition-all ease-in-out duration-150`}>
            <AlertDialog.Title className="mb-2 font-medium">
              Are you absolutely sure?
            </AlertDialog.Title>
            <AlertDialog.Description>
              This will make you more happy and not stress , you can save more
